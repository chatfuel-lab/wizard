...
- **WhatsApp, Instagram, TikTok and Facebook take links; the web widget does not.** `PlatformOperationLinkPlatform` has four values. A Facebook link deals in one page: Connect only while the bot has no page (a second page is refused on Chatfuel's side), Refresh access only while it has exactly one (the link binds to whichever page the server finds first).

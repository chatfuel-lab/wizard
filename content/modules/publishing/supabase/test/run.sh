#!/bin/sh
# Dry-run the Instagram migration + every cf_pub_* RPC on a throwaway local Postgres.
#
...
# storage.buckets/objects, and stubs for pg_cron and pg_net that record what was
# scheduled and what was posted), then every migration runs in order TWICE (they
# must be re-runnable) and scenario.sql exercises every RPC as the role PostgREST
# would use, asserting SQLSTATEs.
#
...
#
#   PGBIN=/opt/homebrew/opt/postgresql@16/bin modules/publishing/supabase/test/run.sh
#   PGDOCKER=my-postgres PGDOCKER_USER=postgres modules/publishing/supabase/test/run.sh
if [ -n "${PGDOCKER:-}" ]; then
  DOCKER_USER="${PGDOCKER_USER:-postgres}"
  REMOTE="/tmp/cf-instagram-test"
  ...
  SOCK="${PGSOCK:-/tmp/cfpg-instagram-test}"
  PORT="${PGPORT:-55433}"

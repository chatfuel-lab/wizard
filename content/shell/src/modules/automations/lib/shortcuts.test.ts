...
const ALL_BINDINGS = WORKSPACE_BINDINGS;
...
describe('the bindings themselves', () => {
  ...
  it('keeps ⌘K and ⌘S reachable from a text field and nothing else', () => {
    const always = WORKSPACE_BINDINGS.filter((binding) => binding.scope === 'always');
    expect(always.map((binding) => binding.id).sort()).toEqual(['palette', 'save']);
  });

  it('does not collide inside one scope', () => {
    for (const group of [WORKSPACE_BINDINGS]) {

...
Fifteen. `core` is installed with everything and has no interface of its own; the rest each
...
| `publishing` | yes | Instagram feed photos, Reels, Stories and carousels, published now or queued. Posting on the spot needs nothing else; **scheduling** needs the `auth` module and `ADMIN_PASSWORD`, because the schedule is one row belonging to the deployment rather than to any workspace in it. |
| `broadcasts` | yes | WhatsApp campaigns over the flow API: a campaign list with a side panel, a five-step composer that picks an approved template and fills its blanks against a phone preview with contact-field personalisation, an audience builder with a live recipient count, send now, later or on a repeat in the bot's own timezone, and a read-only template catalog with a hand-off to WhatsApp Manager. No delivery figures — the API has none. |
| `channels` | yes | Every channel connected to the bot — WhatsApp, Instagram, TikTok, Facebook pages, the web widget — with Disconnect, and the one-shot links that let somebody without dashboard access connect a new WhatsApp, Instagram, TikTok or Facebook channel or refresh an existing one's permissions. |
| `auth` | opt-in | Sign-in for the people who use the app: Supabase email and password, one workspace per account with as many bots inside it as the plan allows, owner/admin/member, invite links, a Team page. Turns the proxy into a gate. |
